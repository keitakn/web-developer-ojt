...
先程紹介したBookインターフェースは本を構成する為に必要なオブジェクトを複数所持しています。

あるオブジェクトに別のオブジェクトを取り込んで扱う事をコンポジションと言います。

純粋な継承だけだとインターフェースが肥大化したり無駄に大きなクラスが出来てしまいメンテナンス性が低下します。

継承とコンポジション、どちらでもやりたい事を実現出来る場合はコンポジションを優先して使うと良いです。
