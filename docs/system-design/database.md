...
さらに高度なテクニックとして `rows_examined` を見て性能を判断する方法もあります。

詳しくは [MySQLのクエリの良し悪しはrows_examinedで判断する](http://blog.kamipo.net/entry/2018/03/22/084126) をご覧下さい。（非常に良い記事です）

データを1件返すだけの比較的単純なSQLであれば `EXPLAIN` で十分です。

複数件のデータを返す可能性があり、かつ性能が求められる場合は上記記事で紹介されている `rows_examined` を確認する事も検討してみて下さい。
...
この4点を徹底するだけでもかなり良いテーブル設計にする事が出来ます。
